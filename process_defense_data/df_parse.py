...
def parse_trace_open_world(full_dir):
    ...
    for file in files:
        full_file = full_dir + "/" + file
        logger.info("parse file: %s", full_file)
        cmd = 'tshark  -r ' + full_file + '  -T fields -Y "tcp.port==' + remote_port + '" -E separator=, -e ip.src -e tcp.srcport -e ip.dst -e tcp.dstport  -e frame.time_epoch -e frame.len > ' + save_filepath + str(count) + '.csv'
        os.system(cmd)
        count += 1
        logger.info(cmd)
...
def extract_trace_open_world(input_filepath, output_filepath):
    ...
    :return:
    '''
    remote_port_list = ["60868", "60858", "60869", "12333"]
...
def extract_feature():
    ...
    data.to_csv("/media/zyan/文档/毕业设计/code/dataset/round13/df_tcp_5000.csv", index=False, header=False)
...
def extract_feature_open_world(filepath):
    ...
    print(len(last_list))
    data = pd.DataFrame(data=last_list)
    data.to_csv("/media/zyan/文档/毕业设计/code/dataset/round10/df_OW_800.csv", index=False, header=False)
...
if __name__ == '__main__':
    ...
    # parse_trace_open_world('/media/zyan/文档/毕业设计/code/第10轮收集/openworld/round0')
